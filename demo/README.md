...
> Other than +page, +layout and +server (not used in this project) files are ignored by sveltekit. Sveltekit recommends commonly used components and modules to be put in `lib` folder, which this project probably not follows, as I can see some svelte components `Counter.svelte` and `Header.svelte`. I fixed this myself in [e5abc03](https://github.com/aslamcodes/learn-svelte/commit/e5abc0328668745821bde2e7dd65358815138f46). But I learned only common files need to be put in lib other components are good to be kept in the routes folder
